...
<ol class="toolbar">
    <li class="btn-item"><a title="Nuevo" href="/sistema/categoria-nuevo" class="fa fa-plus-circle" aria-hidden="true"><span>Nuevo</span></a></li>
...
<script>
function fsalir(){
    location.href ="/sistema/categoria-nuevo";
}
</script>
@endsection
@section('contenido')
                ...
                <div class="form-group col-lg-6">
                    ...
                    <select id="lstMenuPadre" name="lstMenuPadre" class="form-control">
                        ...
                        @for ($i = 0; $i < count($array_categorias); $i++)
                            @if (isset($menu) and $array_categorias[$i]->idmenu == $menu->id_padre)
                                <option selected value="{{ $array_categorias[$i]->idmenu }}">{{ $array_categorias[$i]->nombre }}</option>
                            @else
                                <option value="{{ $array_categorias[$i]->idmenu }}">{{ $array_categorias[$i]->nombre }}</option>
                            @endif
                        @endfor
                    </select>
                </div>
